    ...
    >
      <div
        className={`board-grid grid grid-cols-5 ${
          compact ? "gap-1.5 md:gap-2.5" : "gap-2.5 md:gap-3.5"
        }`}
      >
        {CATEGORIES.map((cat) => (
          <div
          ...
          >
            <span
              className={`leading-tight block-text-header ${
                compact ? "text-[0.6rem] sm:text-[0.65rem] md:text-xs" : "text-[0.68rem] sm:text-xs md:text-sm"
              }`}
            >
              {cat}
            </span>
          ...
          CATEGORIES.map((cat) => {
            ...
            return (
              <button
                ...
                className={`
                  aspect-[4/3] rounded-xl ${compact ? "text-xl md:text-2xl" : "text-2xl md:text-3xl"}
